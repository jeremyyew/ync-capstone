...
Lemma about_visit_fac_v2:
    forall (n' : nat),
      S n' * visit_fac_v2 n' 1 = visit_fac_v2 n' (S n' * 1).
Proof.
  intro n'.
  induction n' as [ | n' IHn'].

  rewrite -> (unfold_visit_fac_v2_0 1).
  rewrite -> (unfold_visit_fac_v2_0 (1 * 1)).
  reflexivity.
...
Lemma about_visit_fac_v2':
    forall (n' a : nat),
      S n' * visit_fac_v2 n' a = visit_fac_v2 n' (S n' * a).
Proof.
  intro n'.
  induction n' as [ | n' IHn'].

  intro a.
  rewrite -> (unfold_visit_fac_v2_0 a).
  rewrite -> (unfold_visit_fac_v2_0 (1 * a)).
  reflexivity.

  assert (H_unfold_visit_fac_v2_S := unfold_visit_fac_v2_S).
  (*apply H_unfold_visit_fac_v2_S in IHn'.*)
...
Proof.
  ...
  rewrite -> (unfold_visit_fib_v2_S (S n'') 0 1).

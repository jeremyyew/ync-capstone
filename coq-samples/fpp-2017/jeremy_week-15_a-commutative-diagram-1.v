...
Proof. 
  ...
  rewrite -> (S_f_Plus_nat_nat ae1 ae2 n1 n2).
  ...
  rewrite -> (S_f_Plus_nat_msg ae1 ae2 n1 s2).
  ...
  rewrite -> (S_f_Plus_msg ae1 ae2 s1).
  ...
  rewrite -> (S_f_Minus_nat_nat_ltbt ae1 ae2 n1 n2).
  ...
  apply H_f_ae2.
  apply H_ltb.
  rewrite -> (S_f_Minus_nat_nat_ltbf ae1 ae2 n1 n2).
  ...
  apply H_f_ae2.
  apply H_ltb.
  rewrite -> (S_f_Minus_nat_msg ae1 ae2 n1 s2).
  ...
  apply H_f_ae1.
  apply H_f_ae2.
  rewrite -> (S_f_Minus_msg ae1 ae2 s1).
...
Proof.
  ...
  rewrite -> (S_f_bci_OK bci bcis' ds ds').
  rewrite -> (S_g_bci_OK bci bcis' ds ds').
  rewrite -> IH_bcis.
  reflexivity.
  exact H_bci_ds.
  exact H_bci_ds.

  rewrite -> (S_f_bci_KO bci bcis' ds s).
  rewrite -> (S_g_bci_KO bci bcis' ds s).
  reflexivity.
  exact H_bci_ds.
  exact H_bci_ds.
Qed.
...
Proof.
  ...
  split.
    ...
    rewrite -> (there_is_at_most_one_decode_execute Top.decode_execute decode_execute).
    rewrite -> (H_bci_ds_OK).
    reflexivity.
    exact decode_execute_satisfies_the_specification_of_decode_execute.
    exact S_decode_execute.
...
Fixpoint run (tp : target_program): expressible_value :=
  match tp with
  | Target_program bcis =>
...
Proof.
  ...
  split.
    ...
    unfold run.
    rewrite -> (there_is_at_most_one_fetch_decode_execute_loop Top.fetch_decode_execute_loop fetch_decode_execute_loop).
    rewrite -> H_fetch_OK.
    reflexivity.
    exact fetch_decode_execute_loop_satisfies_the_specification.
    exact S_fetch_decode_execute_loop.
  split.
...
Proof.
  ...
  intros S_f S_g sp.
  case sp as [ae].
  rewrite -> (S_f compile_aux). 
  rewrite -> (S_g compile_aux). 
  reflexivity.
  exact compile_aux_satisfies_the_specification_of_compile_aux.
  exact compile_aux_satisfies_the_specification_of_compile_aux.
Qed.
...
Proof.
  ...
  unfold compile.
  rewrite -> (there_is_at_most_one_compile_aux Top.compile_aux compile_aux).
  reflexivity.
  exact compile_aux_satisfies_the_specification_of_compile_aux.
  exact S_compile_aux.
Qed.

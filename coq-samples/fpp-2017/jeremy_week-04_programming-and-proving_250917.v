...
Definition test_visit_fib_v2 (f : nat -> nat -> nat -> nat) :=
  ...
  (f 0 2 3 =n= 2)&&
  (***)
  (f 3 0 1 =n= f 2 0 1 + f 1 0 1)&&
  (f 3 1 1 =n= f 2 1 1 + f 1 1 1).
...
Compute test_visit_fib_v3 visit_fib_v3.
...
Lemma unfold_visit_fib_v3_S' :
  forall (n': nat),
   visit_fib_v3 (S n') = let (a1, a2) := visit_fib_v3 n' in (a2, a1 + a2).
Proof.
  unfold_tactic visit_fib_v3.
Qed.
...
Lemma about_visit_fib_v2:
  forall (n a1 a2: nat),
    visit_fib_v2 (S (S n)) a1 a2 = (visit_fib_v2 (S n) a2 (a1 + a2))
                                   +
                                   (visit_fib_v2 n (a1 + a2) (a2 + a1 + a2)).
Admitted.

Lemma about_visit_fib_v2':
  forall (n a1 a2: nat),
    visit_fib_v2 (S (S n)) a1 a2 = visit_fib_v2 (S n) a1 a2 + visit_fib_v2 n a1 a2.
Admitted.

Lemma about_visit_fib_v2'':
  forall (n a1 a2: nat),
    fib_v1 (S n) = visit_fib_v2 (S n) a1 a2
    ->
    fib_v1 n = visit_fib_v2 n a1 a2.
Proof.
  intros n a1 a2.
  intro IH_Sn.
Admitted.
...
Lemma about_visit_fib_v2_and_fib_v1_double :
  forall (k y: nat),
  visit_fib_v2 k (fib_v1 y) (fib_v1 (S y)) = fib_v1 (k + y)
  /\
  visit_fib_v2 (S k) (fib_v1 y) (fib_v1 (S y)) = fib_v1 ((S k) + y).
Proof.
  intros k y.
  split.
  induction k as [ | k' IHk'].
  rewrite -> (unfold_visit_fib_v2_0 (fib_v1 y) (fib_v1 (S y))).
  Search (0 + _ = _).
  rewrite -> (Nat.add_0_l y).
  reflexivity.

(*  rewrite -> (unfold_visit_fib_v2_S k' (fib_v1 y) (fib_v1 (S y))).

  
  rewrite -> (unfold_visit_fib_v2_S 0 (fib_v1 y) (fib_v1 (S y))).
  rewrite -> (unfold_visit_fib_v2_0 (fib_v1 (S y)) (fib_v1 y + fib_v1 (S y))).
  Search (1 + _ = S _).
  rewrite -> (Nat.add_1_l y).
  reflexivity.*)

Abort.

Lemma about_visit_fib_v2_and_fib_v1':
  forall (k y: nat),
    visit_fib_v2 k (fib_v1 y) (fib_v1 (S y)) = fib_v1 (k + y).
Proof.
  intro k.
  ...
  intro y.
  rewrite -> unfold_visit_fib_v2_S.
  Check (IHk' (S y)).
  rewrite -> (Nat.add_comm (fib_v1 y) _).
...
Proof.
  ...
  assert (H_B := about_visit_fib_v2_and_fib_v1').

  Check (about_visit_fib_v2_and_fib_v1' (S n') 0).
  Search (_ + 0 = _).
  rewrite <- (Nat.add_0_r (S n')).
  rewrite <- (about_visit_fib_v2_and_fib_v1' (S n') 0).
  rewrite -> (unfold_fib_v1_0).
  rewrite -> (unfold_fib_v1_1).
  rewrite -> (Nat.add_0_r (S n')).
  reflexivity.
Qed.
(******************)
...
Proof.
  intro ys.
  apply unfold_append_list_nat_nil_nat.
Qed.

Lemma append_xs_nil_nat :
  forall xs : list_nat,
...
Proof.
  ...
  rewrite -> (unfold_append_list_nat_cons_nat x xs nil_nat).
  rewrite -> IHxs.
  reflexivity.
Qed.
...
Proof.
  ...
  rewrite -> (append_nil_nat_ys (append_list_nat ys zs)).
  rewrite -> (append_nil_nat_ys ys).
  reflexivity.

  induction ys as [ | y].
  rewrite -> (append_nil_nat_ys zs).
  rewrite -> (append_xs_nil_nat (cons_nat x xs)).
  reflexivity.

  induction zs as [ | z].
  rewrite -> (append_xs_nil_nat (cons_nat y ys)).
  rewrite -> (append_xs_nil_nat (append_list_nat (cons_nat x xs) (cons_nat y ys))).
  reflexivity.

  rewrite -> (unfold_append_list_nat_cons_nat y ys (cons_nat z zs)).
  rewrite -> (unfold_append_list_nat_cons_nat x xs (cons_nat y (append_list_nat ys (cons_nat z zs)))).

  rewrite -> (unfold_append_list_nat_cons_nat x xs (cons_nat y ys)).
  rewrite -> (unfold_append_list_nat_cons_nat x (append_list_nat xs (cons_nat y ys)) (cons_nat z zs)).
...
Lemma unfold_length_list_nat_v1_nil_nat :
    length_list_nat_v1 nil_nat = 0.
Proof.
  unfold_tactic length_list_nat_v1.
Qed.

Lemma unfold_length_list_nat_v1_cons_nat :
  forall (x : nat) (xs': list_nat), 
    length_list_nat_v1 (cons_nat x xs') = S (length_list_nat_v1 xs').
Proof.
  unfold_tactic length_list_nat_v1.
Qed.


Lemma unfold_visit_length_list_nat_v2_nil_nat :
  forall (a: nat),
    visit_length_list_nat_v2 nil_nat a = a.
Proof.
  unfold_tactic visit_length_list_nat_v2.
Qed.

Lemma unfold_visit_length_list_nat_v2_cons_nat :
  forall (x a : nat) (xs': list_nat), 
    visit_length_list_nat_v2 (cons_nat x xs') a = (visit_length_list_nat_v2 xs' (S a)).
Proof.
  unfold_tactic visit_length_list_nat_v2.
Qed.
...
(***********************)
Lemma about_visit_length_list_nat_v2 :
  forall (xs : list_nat) (a: nat),
    S (visit_length_list_nat_v2 xs a) = visit_length_list_nat_v2 xs (S a).
Proof.
  intros xs.
  induction xs as [ | x].

  intro a. 
  rewrite -> (unfold_visit_length_list_nat_v2_nil_nat a).
  rewrite -> (unfold_visit_length_list_nat_v2_nil_nat (S a)).
  reflexivity.

  intro a.
  rewrite -> (unfold_visit_length_list_nat_v2_cons_nat x a xs).
  rewrite -> (unfold_visit_length_list_nat_v2_cons_nat x (S a) xs).
  rewrite -> (IHxs (S a)).
  reflexivity.
Qed.  
...
Proof.
  unfold length_list_nat_v2.
  intro xs.
  induction xs as [ | x].

  rewrite -> unfold_length_list_nat_v1_nil_nat.
  rewrite -> unfold_visit_length_list_nat_v2_nil_nat.
  reflexivity.

  rewrite -> (unfold_length_list_nat_v1_cons_nat x xs).
  rewrite -> (unfold_visit_length_list_nat_v2_cons_nat x 0 xs).
  rewrite -> IHxs.
  rewrite -> (about_visit_length_list_nat_v2 xs 0).
  reflexivity.
Qed.
